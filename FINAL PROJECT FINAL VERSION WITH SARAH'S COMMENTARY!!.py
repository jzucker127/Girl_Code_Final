#welcome to our humble code! lol jk it's the best code you've ever laid eyes on 
#however! we'd appreciate it if you run it and PLAYYY!!
#but yeah if you can't do that enjoy the commentary..

#so yeah here we import some things..
import pygame
import random
import math
import sys
...
# so this is our tree class, for the trees moving in the background
class Tree():
...
#these are the lines on the road that move in the girls' opposite direction to show that they're walkin forward "smart ikr"
class Lines():
...
#we made lists of the objects we're using so that its easier to apply the methods on all of them to make them move..
trees = []
for i in range (0, 3):
    trees.append(Tree(250+360*i, 350+360*i, 450+360*i, 410, 280))
tree = Background(trees)

#more lists for our objects..
lines = []
for i in range(0,11):
...
#A class for the coders!! the template that makes our girls
class coders():
    def __init__(self, skin_col, interest, hair_type, hair_col, xpos):
        self.skin_col = skin_col #obviously the skin colour
        self.interest = interest #interest determines the outfit she's wearing which is supposed to represent a variety of personalities and how coding could work with any interest
        self.hair_col = hair_col #obviously the hair colour
        self.hair_type = hair_type #it could be 1 for straight hair or 2 for curly
        self.xpos = xpos #since the y position of each character "girl" is contstant the x position changes (so that they're in a line after one another)
        self.pants = skin_col #we made that variable bc the characters legs will need to be animated(to look like she's walking) so it helps to determine
                               #if she's wearing a dress it'll be skin coloured or if she's wearing pants it depends on the outfit (interest)

    #this method draws every object "girl" 
    def draw(self, pos):
        self.pos = pos # pos determines her legs/hands movement we keep switching between 1 and 2 for 2 different leg/arm position to make it
                       #look like they're walking..
        
        #the basic hair behind her head..
        pygame.draw.circle(screen, self.hair_col, (self.xpos - 3, 440), 33)
        pygame.draw.rect(screen, self.hair_col, (self.xpos - 36, 440, 66, 55))
        if self.hair_type == 2: #hair type 2 is curly hair so we just add circles "or curles" over the basic "straight" hair..
        ...
        if self.interest == 1: #if interest is 1 which is the first outfit..
            #dress
        ...
        if self.pos == 1: #these are the 2 different positions of leg/arm and switching between them makes it seem like she's walking/moving
            #legs
...
#different insults or hate messages that we have..
...
class obstacle():
    ...
    def show(self, obs_type):
        if self.num == 1: #so obstacle number 1 is the haters
            #that's the bubble that has their speech
            ...
            if obs_type == 1: #so obstacle number 1 type 1 is the guy hater
                #that's for his shirt and jeans 
            ...
            elif obs_type == 2:# obstacle number 1 type 2 is the girl hater
                #her curls and dress
            ...
            #the hater's "whether 1 or 2" 's eyes and mouth and eyebrows    
        ...
        #obstacle number 2 is syntax errors
        #the language we're using for the errors/questions is javascript..
        elif self.num == 2:
            if obs_type == 1: #number 2 type 1 is a missing semicolon..
            ...
            elif obs_type == 2: #number 2 type 2 is a missing = because i equal is only setting the variable to a value but 2 == is comparing 2 values 
                pygame.draw.ellipse(screen, WHITE, (545, 320, 170, 75))
                pygame.font.init()
                font2 = pygame.font.SysFont(None,40)
        ...
        #obstacle number 3 are questions..
        elif self.num == 3:
            if obs_type == 1: # number 3 type 1 is to find an error, the error is in line 2 because you can't multiply the int by a string in javascript
            ...
            elif obs_type == 2: #number 3 type 2 is would the code run? the answer is no bc we have to initialize i inside the for loop, we forgot "var"
                pygame.draw.ellipse(screen, WHITE, (545, 295, 210, 105))
                pygame.font.init()
...
#starter girl, our first character in the line/movement..
...
#girl coders, a list to keep track of the girls on our screen
girl_coders = [girl]

#variables tp start with..
walking = 1 #that variable keeps track of the walking position.. switching between 1 & 2 in the loop makes the girl look like she's moving..
walkingcount = 1 #count to keep track of how many times we "loop"
earnedscore = False #variable to check whether we arned a new point or not
score = 0 #our score.. lol
obstacle_showing = False #variable to see whether we have an obstacle on the screen or not..
pause_time = 0 #this keeps track of how much time the obstacle was showing on the screen..
start_timer = False #this starts the time of when the obstacle has to show.. and changs back to false when we ran out of time..
number = 0 #the number of obstacle
obs = obstacle(number) #initialising an obstacle..
obs_type = 0 # the type of obstacle.. (obstacles variables are 0 at first because we haven't started yet.. no obstacle is showing now)
keypressedfor = 0 #how many times the key was being kept down
missed = True #whether or not we missed an obstacle..
...
#a loop before our actual game loop for the intro.. where we explain most things
while keypressed < 13:
    # --- Main event loop
...
#our actual game loop.. here we start the game..
# -------- Main Program Loop -----------
while not done:
    # --- Main event loop
    for event in pygame.event.get():
    ...
    if event.type == pygame.KEYDOWN:
        keypressedfor += 1
        earnedscore = False
        # for obstacle number 2 type 1 you have to press the semicolon button ";" to win a point
        if event.key == pygame.K_SEMICOLON and number == 2 and obs_type == 1 and keypressedfor == 1:
            ...
            pause_time = 350
            missed = False
        # for obstacle number 2 type 2 you have to press the equal button "=" to win a point
            ...
            pause_time = 350
            missed = False
        # for obstacle number 1 you have to press the number of the girls you can see on the screen to win a point
            ...
            pause_time = 350
            missed = False
        # for obstacle number 3 type 1 you have to press the number 2 button to win a point
            ...
            pause_time = 350
            missed = False
        # for obstacle number 3 type 2 you have to press the number 2 button to win a point
            ...
            pause_time = 350
            missed = False
        #if you pres any botton while an obstacle is showing that is not a right answer you lose a point..
        elif keypressedfor == 1 and start_timer:
            score = score - 1
        #if the game's over and you lose, you can press the up arrow to try again bc cODERS NEVER GIVE UP!!
            ...
            for i in range(curli):
                girl_coders.pop()
        #if the game's over and you win, you can press the up arrow to play again bc cODERS NEVER STOP CODING!! ALSO THIS GAME IS AMAZING YOU JUST CAN'T STOP!!
    ...
    #the sky and the road basically..
    screen.fill(SKY_BLUE)
    pygame.draw.rect(screen, GRAY, [0, 450, 1000, 150])
    ...
    #adding a point to the score and a girl to your sQUAD!!
    ...
    #drawing every girl in your list
    for i in range(len(girl_coders)):
        girl_coders[i].draw(walking)
    #every 10x60 milliseconds we switch between 1 & 2 to make the girls look like they're moving..
    if walkingcount % 10 == 0:
        if walking == 1:
            walking = 2
        elif walking == 2:
    ...
    #every 500x60 milliseconds we show a random obstacle..
    if walkingcount % 500 == 0:
        obstacle_showing = True

    #initialising the variables for the obstacles..
    if obstacle_showing:
    ...
    #the timer ofr how long an obstacle shows (350x60 milliseconds) ofc if you press the right thing in time, pause time automatically goes to 350
    if start_timer:
        pause_time += 1
        obs.show(obs_type)
        if pause_time >= 350:
    ...
    #what's being displayed if you lose..
    if score < 0:
        walkingcount = 1
        screen.fill(PURPLE)
        pygame.font.init()
    ...
    #and what is displayed if you win..
    if len(girl_coders) == 10:
        ...
        fontA = pygame.font.SysFont(None, 30)
        messageA = fontA.render ("you can try again by pressing the up arrow to get 10 more girls to join the movement ;)", 1, YELLOW)
        screen.blit(messageA, (40, 560))
...
#hope you enjoyed the commentary.. you better have read it ALL because i spent an hour on that.
#our game's the best WOOOOOO YAY US!!
